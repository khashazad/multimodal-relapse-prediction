...
RESULTS_DIR = Path("outputs/results-v2")
CONFIGS_DIR = Path("configs")


def fmt(v):
    """Format a metric value, handling NaN and None."""
    if v is None or (isinstance(v, float) and np.isnan(v)):
        return "NaN"
    return f"{v:.3f}"


def _group_by_sweep(results, sweep_param):
    """Group results by a sweep parameter value."""
    grouped = defaultdict(list)
    for r in results:
        grouped[r["hyperparameters"][sweep_param]].append(r)
    return grouped
...
def load_results(exp_name):
    ...
    files = sorted(OUTPUT_DIR.glob(f"{exp_name}__*.json"))
    results = []
    for fp in files:
        with open(fp) as f:
...
def generate_fold_table(results):
    ...
    lines.append(
        f"| **Mean** | | | **{safe_mean(val_aurocs):.3f}** | | | **{safe_mean(test_aurocs):.3f}** | **{safe_mean(test_auprcs):.3f}** | **{safe_mean(test_f1s):.3f}** |"
    )
...
def generate_calibrated_table(results):
    ...
    for r in sorted(results, key=lambda x: x["fold"]):
        ...
        thresh_str = (
            f"{thresh:.2f}" if isinstance(thresh, (int, float)) else str(thresh)
        )
        lines.append(
            f"| {fold} | {thresh_str} | {fmt(cvf)} | {fmt(cvp)} | {fmt(cvr)} | {fmt(ctf)} | {fmt(ctp)} | {fmt(ctr)} |"
        )
...
def generate_sweep_table(results, sweep_param):
    ...
    grouped = _group_by_sweep(results, sweep_param)

    lines = []
    lines.append(
        f"| {sweep_param} | Mean Val AUROC | Mean Test AUROC | Mean Test AUPRC | Mean Test F1 | Mean Cal Test F1 |"
    )
    lines.append("|---|---|---|---|---|---|")
    ...
    for val in sorted(grouped.keys()):
        ...
        ctf = safe_mean(
            [
                r.get("calibrated_test_metrics", {}).get("f1", float("nan"))
                for r in group
            ]
        )

        lines.append(
            f"| {val} | {fmt(va)} | {fmt(ta)} | {fmt(tp)} | {fmt(tf)} | {fmt(ctf)} |"
        )
...
def generate_aggregate_summary(results):
    ...
    lines.append(
        f"| Val AUROC | {safe_mean(val_aurocs):.3f} | {safe_std(val_aurocs):.3f} |"
    )
    lines.append(
        f"| Test AUROC | {safe_mean(test_aurocs):.3f} | {safe_std(test_aurocs):.3f} |"
    )
    lines.append(
        f"| Test AUPRC | {safe_mean(test_auprcs):.3f} | {safe_std(test_auprcs):.3f} |"
    )
    lines.append(f"| Test F1 | {safe_mean(test_f1s):.3f} | {safe_std(test_f1s):.3f} |")

    cal_f1s = [
        r.get("calibrated_test_metrics", {}).get("f1", float("nan")) for r in results
    ]
    if any(not np.isnan(v) for v in cal_f1s):
        lines.append(
            f"| Cal Test F1 | {safe_mean(cal_f1s):.3f} | {safe_std(cal_f1s):.3f} |"
        )
...
def generate_doc(exp_name):
    ...
    if is_sweep:
        ...
        best_val = max(
            grouped.keys(),
            key=lambda v: safe_mean([r["test_metrics"]["auroc"] for r in grouped[v]]),
        )
...
def generate_comparison_summary():
    ...
    md.append(
        "| Experiment | Mean Val AUROC | Mean Test AUROC | Mean Test AUPRC | Mean Test F1 | Mean Cal Test F1 | Notes |"
    )
    md.append("|---|---|---|---|---|---|---|")
    ...
    for exp_name in all_exps:
        ...
        if sweep_param:
            ...
            best_val = max(
                grouped.keys(),
                key=lambda v: safe_mean(
                    [r["test_metrics"]["auroc"] for r in grouped[v]]
                ),
            )
        ...
        md.append(
            f"| {exp_name} | {fmt(va)} | {fmt(ta)} | {fmt(tp)} | {fmt(tf)} | {fmt(ctf)} | {notes} |"
        )
