#!/bin/bash
# Submit the CNN preprocessing pipeline as a two-phase SLURM job.
#
# Phase 1 — array job (one task per patient):
#   Each task runs:  python -u scripts/preprocess_cnn_data.py --patient <PATIENT_ID>
#   Output:          data/processed_cnn/track<N>/patients/<PATIENT_ID>.pkl
#
# Phase 2 — merge job (runs after ALL array tasks succeed):
#   Runs:  python -u scripts/preprocess_cnn_data.py --merge
#   Output: data/processed_cnn/track<N>/fold_*/  + metadata.json + patient_scalers.pkl
#
# SLURM settings are read from the 'slurm' section of configs/preprocessing_cnn.json.
#
# Usage:
#   bash scripts/submit_preprocess_cnn.sh [--dry-run]

set -e

CONFIG_FILE="configs/preprocessing_cnn.json"
DRY_RUN=false
...
for arg in "$@"; do
    case $arg in
        ...
        --help|-h)
            echo "Usage: submit_preprocess_cnn.sh [--dry-run]"
            echo ""
            echo "Submit the CNN preprocessing pipeline to SLURM."
            echo "SLURM settings are read from configs/preprocessing_cnn.json."
            exit 0
            ;;
...
SLURM_TIME=$(jq -r      '.slurm.time        // "0-06:00"'      "$CONFIG_FILE")
SLURM_MEMORY=$(jq -r    '.slurm.memory      // "24G"'          "$CONFIG_FILE")
SLURM_MERGE_TIME=$(jq -r '.slurm.merge_time // "0-00:30"'      "$CONFIG_FILE")
SLURM_MERGE_MEM=$(jq -r  '.slurm.merge_memory // "16G"'         "$CONFIG_FILE")
SLURM_LOG_DIR=$(jq -r   '.slurm.log_dir    // "outputs/slurm_logs"' "$CONFIG_FILE")
SLURM_JOB_NAME=$(jq -r  '.slurm.job_name   // "preprocess_cnn"'   "$CONFIG_FILE")
...
# ---------------------------------------------------------------------------
# Write patients index file (worker reads by line number)
# ---------------------------------------------------------------------------

mkdir -p "$SLURM_LOG_DIR"
PATIENTS_FILE="$(cd "$(dirname "${SLURM_LOG_DIR}/patients_cnn.txt")" && pwd)/patients_cnn.txt"
...
ARRAY_CMD=(
    ...
    --wrap=". /w/20252/khashazad/env/bin/activate && \
        ...
        python -u scripts/preprocess_cnn_data.py --patient \"\$PATIENT_ID\""
)
...
MERGE_CMD=(
    ...
    $COMMON_OPTS
    --wrap=". /w/20252/khashazad/env/bin/activate && python -u scripts/preprocess_cnn_data.py --merge"
)
