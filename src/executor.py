...
@click.option(
    "--lang", type=str, default="python", help="language to use for the experiment"
)
@click.option(
    "--source", type=str, default="experiment.py", help="name of the source file"
)
def experiment(config, name, index, lang, source):
    ...
    fixed_params = {
        ...
        if not isinstance(v, list) and not is_reserved(k)
    }
    variable_params = {
        k: v
        for k, v in config_data.items()
        if isinstance(v, list) and not is_reserved(k)
    }
    ...
    if index >= len(param_combinations):
        raise ValueError(
            f"Index {index} is out of range. Only {len(param_combinations)} experiments available."
        )
    ...
    experiment_params.update(
        {
            k: param_combinations[index][i]
            for i, (k, v) in enumerate(variable_params.items())
        }
    ...
    exp_name = name
    var_parts = [
        f"{k}={experiment_params[k]}".replace("/", "_") for k in variable_params.keys()
    ]
    ...
    if lang not in ["python", "julia"]:
        raise ValueError(
            f"Language {lang} is not supported. Supported languages are python and julia."
        )
